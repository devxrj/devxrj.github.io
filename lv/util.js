...
(() => {
  ...
  if (get_p("cl") == "1" && loc_get(ls_key)) loc_rem(ls_key);  
...
function get_links()
{
  var ls_conteudo = loc_get(ls_key);
...
{
 if (elem.id == "it_opc3"){
  if (confirm("Clear Cache?")) loc_rem(ls_key); 
 }

 if (elem.id == "it_opc4"){
  if (confirm("Clear Cache All?")) {
   loc_keys().forEach((key)=>{
   if (key.startsWith(ls_prefix)) loc_rem(key);
   });
 }
}
...
var is_frame = false ;
if (o.getAttribute("data-conf") && o.getAttribute("data-conf").toLowerCase().includes("f")) is_frame = true;
...
function op3()
{
Array.from(elem_lv.getElementsByClassName("agrup")).forEach((o) => {
 const m_spn = document.createElement("span");
 m_spn.innerHTML = "&#x21bb;";
 m_spn.style.marginLeft = "10px";
 m_spn.addEventListener("click", (event) => {
  event.stopPropagation();
  var lnks = Array.from(o.nextElementSibling.querySelectorAll("a, span"));
